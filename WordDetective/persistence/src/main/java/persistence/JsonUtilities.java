...
public final class JsonUtilities {
  ...
  /**
   * Constant used to reference the absolute path of the persistence module's resource directory.
   */
  public static final String PATH_TO_RESOURCES = getAbsolutePathAsString();

  /**
   * Provides absolute path to current working directory.
   * Implemented in this fashion due to path differences in working files and test files.
   *
  ...
  public static boolean successfullyAddedUserPersistently(final User user) {
    try (FileWriter fw = new FileWriter(PATH_TO_RESOURCES + "/users/" + user.getUsername() + ".json", StandardCharsets.UTF_8)) {
        GSON.toJson(user, fw);
   ...
   * @param password The password of which to check against the sotred password of the username.
   * @return Boolean indicating if the username's stored password matches with the provided password.
  ...
  */
  ...
  /**
   * Fetches a specific property from an specified file without the need to load in all the file's content.
   ...
   * @param endpoint The final filepath to the directory of which to find the filenames of.
   * @return Set<String> with all filenames in the directory,
   * where the ".json" file extension removed, and all underscores converted to spaces.
